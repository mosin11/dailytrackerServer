...
router.get('/totalPlansCount', authenticateToken, async (req, res) => {
    try {
        const totalPlans = await Plan.countDocuments({ userId: req.user.userId });
        logger.debug("Fetched total plans count successfully");
        res.status(200).json({ totalPlans });
    } catch (err) {
        res.status(500).json({ message: 'Error fetching total plans count', error: err });
    }
});
